...
HTTP 请求对象，由 App 装配，业务通过 `$request` 或 `getApp()->request()` 访问。**Request 不持有路由**：匹配在 `App::run()` 内直接调用 App 持有的 Router，命中参数经 `$request->params->fill()` 注入。

```php
$request = getApp()->request();
$uri = $request->uri();               // 请求 URI
$method = $request->method();         // 请求方法
$params = $request->params;           // 路由参数（App::run() 匹配后注入）
```
...
|------|------|
| `uri()` | 请求 URI |
| `$request->params` | 路由参数集合（App::run() 匹配后经 fill() 注入） |
