...
持有全部校验规则定义方法及规则状态数据，作为 `Rule` 的内部代理。架构上对应 Database/Model 中的 Builder/Query 角色：`Rule`（门面）通过魔术方法委托到本类，本类负责实际的规则存储与链式构建。所有规则方法返回 `$this` 以支持链式调用。

规则存储结构：
- `$rule`：规则名 → 规则值的映射，最终由 `Validator` 消费
- `$errorMessages`：规则名 → 自定义错误信息

## 属性

| 属性 | 类型 | 默认 | 可见性 | 说明 |
|------|------|------|--------|------|
| `$rule` | `array\|null` | `null` | public | 规则定义，键为规则名（`required`/`type`/`min` 等），值为规则参数 |
| `$errorMessages` | `array` | `[]` | public | 自定义错误信息，键为规则名，值为错误文案 |
...
| 参数 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `$value` | `string\|array` | 无 | 数据类型或数据类型数组，如 `"string"`、`["integer", "string"]` |
| `$message` | `string` | `""` | 校验失败时的报错信息 |
...
校验字符串是否包含子串，或数组是否包含指定元素（数组元素含基本类型）。
...
### `minLength($value, $message = "")` — 校验长度是否大于等于指定长度
...
### `maxLength($value, $message = "")` — 校验长度是否小于等于指定长度
...
### `length($min, $max, $message = "")` — 校验长度是否在指定范围内
...
| 参数 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `$callback` | `\Closure\|callable` | 无 | 校验函数，签名 `function($value, $rules, $data): Result\|void`。失败时返回 `Result::failed()`；成功时返回 `Result::succeeded()`、或不返回/返回其他值皆视为通过 |

**返回值**

- `$this`：当前实例（支持链式调用）。

### `useRule(RuleInterface $validateRule)` — 使用别的校验规则
...
可变参数：支持 `in('a', 'b', 'c')` 或 `in(['a', 'b', 'c'], 'message')`。使用非数组传参时，所有参数作为值列表且无自定义消息。

**参数**

| 参数 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `$values` | `array\|string` | 无 | 值列表或值列表+消息 |
| `$message` | `string` | `""` | 校验失败时的报错信息 |
...
```php
Rule::in(['active', 'pending']);
Rule::in('active', 'pending');
Rule::in(['active'], '状态不合法');
```
